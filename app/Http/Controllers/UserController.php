...
class UserController extends Controller
{
        ...
        if ($hruser->office == "AO3" || $hruser->office == "AO4" || $hruser->office == "AO6" || $hruser->office == "AO7")
        {
            if ($hruser->office !== $user->office)
            {
                ...
                if($user->contract == "International")
                {
                    ...
                    if ($hruser->employee_number !== '104588' AND $hruser->employee_number !== '101783' AND $hruser->employee_number !== '108400' AND $hruser->employee_number !== '1001')
                    {
                        abort(403);
                    }
    ...
    public function balanceupdate(Request $request, User $user)
    {
        ...
        else
        {
            ...
            // $user->save();
            // $balancesbefore = Balance::where('user_id', $user->id)->get();
            // dd($balancesbefore);
            // dd($request);
    
            
            // $changes = Balance::where('user_id', $user->id)->findOrFail();
            // $changess = $user->balances();
            // if($changes->isDirty())
            // {
